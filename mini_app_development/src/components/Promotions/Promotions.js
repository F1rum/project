...
const PromotionCard = ({ image, promotion, oldPrice, newPrice, addToCart }) => {
    return (
        <div className={styles.promotionCard}>
            ...
            <button onClick={() => addToCart({ image, product: promotion, price: newPrice })}>
                Добавить в корзину
            </button>
        </div>
    );
...
export default function Promotions() {
    ...
    const groupedPromotions = promotions.reduce((acc, promo) => {
        if (!acc[promo.store]) acc[promo.store] = [];
        acc[promo.store].push(promo);
        return acc;
    }, {});
                ...
                <div key={store} className={styles.storeBlock}>
                    ...
                    <div className={styles.promotionContainer}>
                        {promotions.map((promo, index) => (
                            <PromotionCard key={index} {...promo} addToCart={addToCart} />
                        ))}
                    </div>
                </div>
            ))}
